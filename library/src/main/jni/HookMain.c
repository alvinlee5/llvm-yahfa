...
// Include trampoline.c to not need to configure build system drastically.
// The llvm pass will apply per source file (or translation unit) on compilation, and we don't want redefinitions
...
static int doBackupAndHook(void *targetMethod, void *hookMethod, void *backupMethod) {
    ...
    if (newEntrypoint) {
        memcpy((char *) targetMethod + OFFSET_entry_point_from_quick_compiled_code_in_ArtMethod,
               &newEntrypoint,
               pointer_size);
        int val = memcmp((char *) targetMethod + OFFSET_entry_point_from_quick_compiled_code_in_ArtMethod,
                         &newEntrypoint, pointer_size);
        if (val == 0)
        {
            LOGI("DEBUG - AFTER MEMCPY - Entrypoint: ALL SAME");
        }
        else
        {
            LOGI("DEBUG - AFTER MEMCPY - Entrypoint: DIFFERENT");
        }
    } else {
